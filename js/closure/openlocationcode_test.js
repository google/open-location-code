  ...
  testDecode: function() {
    ...
    xhrIo_.listenOnce(EventType.COMPLETE, () => {
      ...
      for (var i = 0; i < lines.length; i++) {
        const fields = lines[i].split(',');
        const code = fields[0];
        const length = parseFloat(fields[1]);
        ...
        assertRoughlyEquals('testDecode ' + 1, latLo, gotCodeArea.latitudeLo, 1e-10);
        assertRoughlyEquals('testDecode ' + 1, lngLo, gotCodeArea.longitudeLo, 1e-10);
        assertRoughlyEquals('testDecode ' + 1, latHi, gotCodeArea.latitudeHi, 1e-10);
        assertRoughlyEquals('testDecode ' + 1, lngHi, gotCodeArea.longitudeHi, 1e-10);
        asyncTestCase.continueTesting();
      }
    });
    asyncTestCase.waitForAsync('Waiting for xhr to respond');
