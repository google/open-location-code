...
if [ $? -ne 0 ]; then
  if [ -z "$TRAVIS" ]; then
    ...
    go run ../travis-utils/github_comments.go --pr "$TRAVIS_PULL_REQUEST" \
        --comment '**Project has format errors that must be fixed**. Here is a diff, or run `checks.sh`:'"<br><pre>$SPOTLESS</pre>" \
...
if [ $? -ne 0 ]; then
  if [ -z "$TRAVIS" ]; then
    ...
    RETURN=1
    go run ../travis-utils/github_comments.go --pr "$TRAVIS_PULL_REQUEST" \
        --comment '**File has format errors that must be fixed**. Here is the report:'"<br><pre>$ANALYSIS</pre>" \
        --commit "$TRAVIS_PULL_REQUEST_SHA"
  fi
fi
