...
struct EncodingTestData {
  ...
  double lat_deg;
  double lng_deg;
  long long int lat_fixed;
  long long int lng_fixed;
  size_t length;
  std::string code;
};
...
std::vector<EncodingTestData> GetEncodingDataFromCsv() {
  ...
  for (size_t i = 0; i < csv_records.size(); i++) {
    ...
    test_data.lat = csv_records[i][0];
    test_data.lng = csv_records[i][1];
    test_data.lat_deg = strtod(csv_records[i][0].c_str(), nullptr);
    test_data.lng_deg = strtod(csv_records[i][1].c_str(), nullptr);
    test_data.lat_fixed = strtoll(csv_records[i][2].c_str(), nullptr, 10);
    test_data.lng_fixed = strtoll(csv_records[i][3].c_str(), nullptr, 10);
