...
#define ASSERT_INT_EQ(var, num) \
    do { \
        ...
        if (!assert_int_ok) { \
            fprintf(stderr, "%s %d [%s] != [%ld] (%ld)\n", __FILE__, __LINE__,  #var, (long) num, (long) var); \
            abort(); \
        } \
    } while (0)
...
#define ASSERT_FLT_EQ(var, num) \
    do { \
        int assert_flt_ok = ((unsigned long long) (var * OLC_SCALE)) == ((unsigned long long) (num * OLC_SCALE)); \
        if (!assert_flt_ok) { \
            fprintf(stderr, "%s %d [%s] != [%lf] (%lf)\n", __FILE__, __LINE__,  #var, (double) num, (double) var); \
            abort(); \
        } \
    } while (0)
