...
static void encode_len(void) {
  ...
  std::string code =
      openlocationcode::Encode({data_pos_lat, data_pos_lon}, data_pos_len);
...
static void recover(void) {
  ...
  std::string recovered_code = openlocationcode::RecoverNearest(
      data_code_6, {data_ref_lat, data_ref_lon});
