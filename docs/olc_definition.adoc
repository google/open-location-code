...
. Only Open Location Codes with at least eight digits and without padding
may be shortened by omitting leading digits. Short codes must always include
the "+" character and must never include padding "0" characters.
...
. RTL (right-to-left) languages should be handled as follows:
